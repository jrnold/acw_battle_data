...
def battles(data, dst, filename):
    with open(path.join(dst, filename), 'w', encoding='utf8') as f:
        fieldnames = ('battle_id', 'theater', 'theater_years', 'start_date',
                      'end_date', 'result', 'page')
...
def commanders(data, dst, filename):
    with open(path.join(dst, filename), 'w', encoding='utf8') as f:
        fieldnames = ('battle_id', 'belligerent', 'commander_number',
                      'PersonID', 'last_name', 'first_name', 'middle_name',
...
def build(src, dst):
    ...
    cwsac_links(
        path.join(src, "rawdata", "clodfelter2008",
                  'clodfelter_to_cwsac.yaml'),
        path.join(dst, 'clodfelter_to_cwsac.json'))
    dbpedia_links(
        path.join(src, "rawdata", "clodfelter2008",
                  'clodfelter_to_dbpedia.yaml'),
