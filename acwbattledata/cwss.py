...
def theaters_csv(root, dst):
    ...
    with open(dst, 'w', encoding='utf8') as f:
        ...
        theaters_rows = [{
            'TheaterCode': k,
            'TheaterName': v
        } for k, v in sorted(theaters.items())]
...
def campaigns_csv(root, dst):
    ...
    print("Writing: %s" % dst)
    with open(dst, 'w', encoding='utf8') as f:
        writer = csv.DictWriter(f, ('CampaignCode', 'CampaignName',
                                    'CampaignDates', 'CampaignStartDate',
                                    'CampaignEndDate', 'TheaterCode'))
        writer.writeheader()
        for i, entry in enumerate(root.findall('.//%s' % xmlns('entry'))):
            properties = entry.find('./%s/%s' % (xmlns('content'),
                                                 xmlns('m:properties')))
            ...
            ) != '' and campaign_code not in campaigns:
...
def commander_csv(root, dst):
    fields = ('BattlefieldCode', 'belligerent', 'commander_number',
              'commander')
    print("Writing: %s" % dst)
    with open(dst, 'w', encoding='utf8') as f:
        ...
        for entry in root.findall('.//%s' % xmlns('entry')):
            ...
            for i, ordinal in enumerate(("First", "Second", "Third")):
                if properties.find(xmlns("d:%sEnemyCommander" %
                                         ordinal)).text is not None:
                    if re.search(
                            "\\S",
                            properties.find(
                                xmlns("d:%sEnemyCommander" % ordinal)).text):
                        writer.writerow({
                            'BattlefieldCode':
                            battlecode.upper(),
                            'belligerent':
                            ...
                            guid_clean(
                                properties.find(
                                    xmlns("d:%sEnemyCommander" % ordinal))
                                .text)
                        })
                if properties.find(
                        xmlns("d:%sUSCommander" % ordinal)).text is not None:
                    if (battlecode == "MO026" and ordinal == "Second"):
                        ...
                        writer.writerow({
                            ...
                            2,
                            "commander":
                            "90849b39-6f3a-4d88-af29-ddded5d1733a"
                        })
                    else:
                        if re.search(
                                "\\S",
                                properties.find(
                                    xmlns("d:%sUSCommander" % ordinal)).text):
                            writer.writerow({
                                'BattlefieldCode':
                                battlecode.upper(),
                                'belligerent':
...
def people_csv(root, dst):
    fields = ("PersonID", "ID", "LastName", "Suffix", "FirstName",
              "MiddleName", "MiddleInitial", "Rank", "Bio", "BioSource",
              "NarrativeLink1", "NarrativeLink2")
    with open(dst, 'w', encoding='utf8') as f:
        print("Writing: %s" % dst)
        writer = csv.DictWriter(f, fields)
        writer.writeheader()
    ...
    with open(dst, 'w', encoding='utf8') as f:
        ...
        for i, entry in enumerate(root.findall('.//%s' % xmlns('entry'))):
            ...
            replacements = (
                ...
                ('"Brigadier General,', '"Brigadier General",'),
                ('"European American"Major General"',
                 '"European American", "Major General"'),
                ('European American" Commanders',
                 'European American", Commanders'),
                ('"KS - Slave State, "John Brown",',
                 '"KS - Slave State", "John Brown",'),
                ('"European American" Minister',
                 '"European American", Minister'),
                ('"European American "Commanders',
                 '"European American", Commanders'),
                ...
                ('"CS - Commander - VA064,"CS - Commander - VA114"',
                 '"CS - Commander - VA064", "CS - Commander - VA114"'), )
            for strfrom, strto in replacements:
                keywords = keywords.replace(strfrom, strto)
            if id in (
                    "ecb8a078-2c3c-4056-b2b5-013fe3e91fa3",
...
def battleunitslink_csv(root, comments, dst):
    fields = ('BattlefieldCode', 'Comment', 'Source', 'UnitCode', 'companies',
              'batteries', 'detachment', 'section')
    with open(dst, 'w', encoding='utf8') as f:
        print("Writing: %s" % dst)
        writer = csv.DictWriter(f, fields)
        writer.writeheader()
        for i, entry in enumerate(root.findall('.//%s' % xmlns('entry'))):
            ...
            for fld in ('BattlefieldCode', 'Comment', 'Source', 'UnitCode'):
                try:
                    row[fld] = properties.find(
                        xmlns('d:%s' % fld)).text.strip()
...
def build_units(src, dst):
    ...
    data.replace({'side': {'U': 'US', 'C': 'Confederate'}}, inplace=True)
    # Fix bad funcs
    data.loc[data['unit_code'].isin(('CAL0105R', 'CVA0108R0M')), 'func'] = None
    data.duplicate = data.duplicate.fillna(0).astype('int64')
    for x in ('pubdate', 'updatedate', 'updateby', 'comments', 'updatetype',
              'isdelete', 'longhistory', 'history', 'arm'):
        del data[x]
...
def build_state_names(src, dst):
    ...
    data = pandas.read_csv(srcfile, sep='\t')
    data.rename(
        columns=lambda k: k.lower().replace('state_', ''), inplace=True)
    data = data.query('~ isdelete')
    for x in ('updateby', 'updatedate', 'updatetype', 'pubdate', 'isdelete',
              'comments'):
        del data[x]
...
def build_unit_titles(src, dst):
    ...
    for col in ('updatedate', 'updateby', 'updatetype', 'isdelete', 'comments',
                'pubdate'):
        del newdata[col]
    newdata.replace({'side': {'C': 'Confederate', 'U': 'US'}})
    print("Writing: %s" % dstfile)
...
def build_category(src, dst):
    ...
    for col in ('updatedate', 'updateby', 'isdelete', 'updatetype', 'comments',
                'pubdate', 'pk_id'):
        del data[col]
    print("Writing: %s" % dstfile)
    data.to_csv(dstfile, index=False)
...
def build(src, cwssdir, dst):
    ...
    comments = {}
    with open(
            path.join(src, 'rawdata', 'cwss', 'battle_units.csv'),
            'r',
            encoding='utf8') as f:
