...
def build(src, dst):
    with open(
            path.join(src, "rawdata", "aad", "events.json"), 'r',
            encoding="utf8") as f:
        data = json.load(f)

    rows = []
    for k, v in sorted(
            data.items(), key=lambda x: x[1]["Reference Number"]["value"]):
        row = {}
        row["url"] = k
        ...
        row["ownership_federal"] = yn(
            v["Ownership: Federal Government"]["value"])
        ...
        row["interpretive_political"] = yn(v[
            "Interpretive Potential: Effect Upon Regional or State Political Situation"]
                                           ["value"])
        row["interpretive_commander_loss"] = yn(v[
            "Interpretive Potential: Loss of Significant Commander (Wounding, Death, Relieved of Command)"]
                                                ["value"])
        row["interpretive_casualties"] = yn(
            v["Interpretive Potential: Unusually High Casualties"]["value"])
        row["interpretive_tactics_strategy"] = yn(v[
            "Interpretive Potential: Illustrates Important Lessons in Military Tactics and Strategy"]
                                                  ["value"])
        row["interpretive_public_mind"] = yn(v[
            "Interpretive Potential: Unusual Importance in the Public Mind and Imagination"]
                                             ["value"])
        row["interpretive_combat_arm"] = yn(v[
            "Interpretive Potential: Significant Participation of Cavalry, Artillery, or Other Single Combat Arm"]
                                            ["value"])
        row["interpretive_military_firsts"] = yn(
            v["Interpretive Potential: Military Firsts"]["value"])
        row["interpretive_minority_troops"] = yn(v[
            "Interpretive Potential: Participation of Significant Numbers of Minority Troops"]
                                                 ["value"])
        row["interpretive_economic"] = yn(
            v["Interpretive Potential: Significant Economic Consequences"][
                "value"])
        row["interpretive_archaelolgical"] = yn(v[
            "Interpretive Potential: High Archaeological Potential"]["value"])
        row["interpretive_logistics"] = yn(v[
            "Interpretive Potential: Unusually Significant Logistics or Supply Feat"]
                                           ["value"])
        row["interpretive_individual_bravery"] = yn(v[
            "Interpretive Potential: Exceptional Individual Initiative in Bravery or Command"]
                                                    ["value"])
        row["interpretive_group_behavior"] = yn(
            v["Interpretive Potential: Exceptional Group Behavior"]["value"])
        row["interpretive_joint_ops"] = yn(v[
            "Interpretive Potential: Illustrates Joint Operations (Army_Navy)"]
                                           ["value"])
        row["interpretive_coop_armies"] = yn(v[
            "Interpretive Potential: Illustrates Cooperation of Separate Military Departments or Armies"]
                                             ["value"])
        row["interpretive_naval"] = yn(
            v["Interpretive Potential: Naval Operations"]["value"])
