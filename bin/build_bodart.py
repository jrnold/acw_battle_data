...
def battle_csv(src, dst):
    ...
    fields = [
        ...
        'category_belagerung', 'category_kapitulation', 'category_einnahme',
        'category_size', 'page'
    ]
    ...
    with open(dst, 'w', encoding='utf-8') as f:
        ...
        for row in data:
            ...
            for category in CATEGORIES:
                row['category_' + category.lower()] = int(
                    category in row['category'])
            writer.writerow(row)
    print("Writing: %s" % dst)


fields_forces = ("battle_id", "belligerent", "strength", "strength_engaged",
                 "infantry", "cavalry", "artillery", "guns", "killed",
                 "killed_percent", "killed_generals", "killed_officers",
                 "killed_wounded", "killed_wounded_percent",
                 "killed_wounded_generals", "killed_wounded_officers",
                 "wounded", "wounded_percent", "wounded_generals",
                 "wounded_officers", "captured", "captured_generals",
                 "captured_officers", "missing", "missing_percent",
                 "missing_generals", "missing_officers", "casualties",
                 "casualties_percent", "casualties_officers",
                 "casualties_generals", "losses_guns", "losses_caissons",
                 "losses_cannon", "losses_canons", "losses_flags",
                 "losses_munition_wagons", "losses_wagons", "note")
...
def commanders_csv(src, dst):
    with open(src, 'r', encoding='utf8') as f:
        data = yaml.load(f)
    fieldnames = ('battle_id', 'belligerent', 'full_name', 'last_name',
                  'first_name', 'middle_name', 'suffix', 'rank', 'dbpedia')
    with open(dst, 'w', encoding='utf8') as f:
        writer = csv.DictWriter(f, fieldnames, extrasaction='ignore')
        writer.writeheader()
        for battle in data:
            for side in battle['forces']:
                for i, row in enumerate(battle['forces'][side]['commanders']):
                    ...
                    if 'dbpedia' not in row:
                        ...
                        row['dbpedia'] = "https://dbpedia.org/resource/%s" % row[
                            'dbpedia']
                    writer.writerow(row)
    print("Writing: %s" % dst)
...
def generals_killed_csv(src, dst):
    with open(src, 'r', encoding='utf8') as f:
        data = yaml.load(f)
    fieldnames = ('battle_id', 'belligerent', 'full_name', 'last_name',
                  'first_name', 'middle_name', 'suffix', 'rank', 'date',
                  'dbpedia')
    with open(dst, 'w', encoding='utf8') as f:
        writer = csv.DictWriter(f, fieldnames, extrasaction='ignore')
        writer.writeheader()
            ...
            for side in battle['forces']:
                if 'generals_killed' in battle['forces'][side]:
                    for i, row in enumerate(
                            battle['forces'][side]['generals_killed']):
                        row['battle_id'] = battle['battle_id']
                        row['belligerent'] = side
                        row['rank'] = RANKS[row['rank']]
                        if 'dbpedia' not in row:
...
def build(src, dst):
    ...
    generals_killed_csv(filename,
                        os.path.join(dst, "bodart1908_generals_killed.csv"))
    bodart_to_cwsac(filename, os.path.join(dst, "bodart1908_to_cwsac.json"))
    bodart_to_dbpedia(filename, os.path.join(dst,
                                             "bodart1908_to_dbpedia.json"))
